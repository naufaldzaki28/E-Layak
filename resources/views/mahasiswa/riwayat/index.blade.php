            ...
            <div class="bg-white rounded-xl shadow-sm border border-gray-100 overflow-hidden">
                <div class="overflow-x-auto">
                    <table class="w-full text-left border-collapse min-w-[600px]">
                        <thead class="bg-gray-50 text-gray-600 uppercase text-[10px] md:text-sm font-semibold">
                            <tr>
                        ...
                        <tbody class="divide-y divide-gray-100">
                            @forelse($dataRiwayat as $item)
                                <tr class="hover:bg-gray-50 transition">
                                    <td class="p-4">
                                        @if ($item->jenis == 'Aduan')
                                            <span class="inline-flex items-center gap-2 px-3 py-1 rounded-full text-[10px] md:text-xs font-bold bg-red-50 text-red-600 border border-red-100">
                                                <i class="fas fa-bullhorn"></i> Aduan
                                            </span>
                                        @else
                                            <span class="inline-flex items-center gap-2 px-3 py-1 rounded-full text-[10px] md:text-xs font-bold bg-blue-50 text-blue-600 border border-blue-100">
                                    ...
                                    <td class="p-4">
                                        <p class="text-sm md:text-base font-semibold text-gray-800 truncate max-w-[150px] md:max-w-xs" title="{{ $item->judul }}">
                                            {{ $item->judul }}
                                        </p>
                                        <p class="text-[10px] md:text-xs text-gray-500 truncate max-w-[150px] md:max-w-xs">
                                    ...
                                    <td class="p-4">
                                        @php
                                            ...
                                            if (in_array($status, ['pending', 'diajukan', 'menunggu'])) {
                                                $warna = 'yellow';
                                                $icon = 'fa-clock';
                                            } elseif ($status == 'diproses') {
                                                $warna = 'blue';
                                                $icon = 'fa-spinner fa-spin';
                                            } elseif (in_array($status, ['selesai', 'disetujui', 'success'])) {
                                                $warna = 'green';
                                        ...
                                        <span class="inline-flex items-center gap-1.5 px-3 py-1 rounded-full text-[10px] md:text-xs font-medium capitalize bg-{{ $warna }}-100 text-{{ $warna }}-700 border border-{{ $warna }}-200 whitespace-nowrap">
